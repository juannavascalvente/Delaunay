															...
															double &dist)
{
	...
	if (found)
	{
		...
		do
		{
			...
			if (voronoi.isInnerToArea(p, pointIndex))
			{
				found = true;
#ifdef DEBUG_DELAUNAY_FIND_CLOSESTPOINT
...
* OUT:		faces		list of triangles in the path.
...
{
	bool 	found=false;		// Return value.
	int		nodeIndex=0;		// Index of the node assgiened to face.
