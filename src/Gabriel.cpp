...
***************************************************************************/
void Gabriel::print(std::ostream& out)
{
...
***************************************************************************/
bool Gabriel::read(string fileName)
{
...
***************************************************************************/
bool Gabriel::write(string fileName)
{
...
* 				binary format.
***************************************************************************/
bool Gabriel::readBinary(string fileName)
{
	bool read=false;		// Return value.
	ifstream ifs;			// Input file.
    ...
    if (this->valid)
    {
    	...
    	if (ifs.is_open())
    	{
    		...
    		memcpy( this->validEdge, buffer, sizeof(bool)*size);
    		delete[] buffer;
    		read = this->valid;
    	}
    }
...
* 				in binary format.
***************************************************************************/
bool Gabriel::writeBinary(string fileName)
{
	bool written=false;		// Return value.
	ofstream ofs;			// Output file.
	int	size=0;				// Buffer size.
	int	nElements=0;		// # elements in Gabriel graph.
	char* buffer=NULL;		// Buffer data.

    // Check Gabriel graph allocated.
    if (this->valid)
    {
    	// Open file.
    	ofs.open( fileName.c_str(), std::ofstream::binary);

    	// Check file is opened.
    	if (ofs.is_open())
    	{
    		size = (this->getSize()*sizeof(bool))+sizeof(int);
    		buffer = new char[size];
    		nElements = this->getSize();
    		memcpy( buffer, &nElements, sizeof(int));
    		memcpy( &buffer[4], this->validEdge, sizeof(bool)*this->getSize());
